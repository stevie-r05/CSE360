...
namespace CSE360Project {
	...
	void DB_Users::outputAllUsers() {
		if (!user_data.empty()) {
			for (int i = 0; i < (int) user_data.size(); i++) {
				cout << i+1 << ") ";
				cout << user_data[i].uid << '\t'; //User ID - Autogenerated in this class.
				cout << user_data[i].username << '\t';
				cout << user_data[i].password << '\t';
				cout << user_data[i].lastName << '\t';
				cout << user_data[i].firstName << '\t';
				cout << user_data[i].securityQuestion << '\t';
				cout << user_data[i].securityAnswer << '\t';
